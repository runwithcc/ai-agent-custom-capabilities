...
from gateway.platforms.dailynote_writer import apply_dailynote_result, sync_event_to_dailynote
from gateway.platforms.link_ingest import enrich_text_with_link_content
try:
    from gateway.platforms.lifeos_phase1_runtime import persist_phase1_shadow_records
except ImportError:
    from lifeos_phase1_runtime import persist_phase1_shadow_records
...
_DEFAULT_BATCH_DMS = False
_DEFAULT_ACK_REACTION_IN_DMS = True
...
class FeishuAdapter(BasePlatformAdapter):
    ...
    def _build_event_handler(self) -> Any:
        ...
        return (
            ...
            .register_p2_im_message_recalled_v1(self._on_ignored_event)
            .register_p2_im_chat_access_event_bot_p2p_chat_entered_v1(self._on_ignored_event)
            .register_p2_im_chat_member_bot_added_v1(self._on_bot_added_to_chat)
            .register_p2_im_chat_member_bot_deleted_v1(self._on_bot_removed_from_chat)
            .register_p2_im_chat_member_user_added_v1(self._on_ignored_event)
            .register_p2_im_chat_member_user_deleted_v1(self._on_ignored_event)
            .register_p2_im_chat_member_user_withdrawn_v1(self._on_ignored_event)
            .register_p2_im_chat_updated_v1(self._on_ignored_event)
    ...
    def _on_ignored_event(self, data: Any) -> None:
        """Swallow benign Feishu system events so the websocket SDK stays quiet."""
        header = getattr(data, "header", None)
        event = getattr(data, "event", None)
        event_type = str(getattr(header, "event_type", "") or "")
        message_id = (
            str(getattr(getattr(event, "message", None), "message_id", "") or "")
            or str(getattr(event, "message_id", "") or "")
        )
        chat_id = str(getattr(event, "chat_id", "") or "")
        logger.debug(
            "[Feishu] Ignoring system event type=%s message_id=%s chat_id=%s",
    ...
    def _on_reaction_event(self, event_type: str, data: Any) -> None:
        ...
        if (
            ...
            or emoji_type in set(self._ack_emoji_candidates())
            or not message_id
    ...
    @staticmethod
    def _parse_ack_emoji_candidates(raw: str) -> List[str]:
    ...
    def _ack_emoji_candidates(self) -> List[str]:
        raw_many = os.getenv("HERMES_FEISHU_ACK_EMOJIS", "").strip()
        raw_one = os.getenv("HERMES_FEISHU_ACK_EMOJI", "").strip()
        candidates = self._parse_ack_emoji_candidates(raw_many or raw_one)
        return candidates or list(_DEFAULT_ACK_EMOJIS)

    def _pick_ack_emoji(self) -> str:
        candidates = self._ack_emoji_candidates()
        if not getattr(self, "_ack_emoji_pool_logged", False):
            source = "env" if (os.getenv("HERMES_FEISHU_ACK_EMOJIS", "").strip() or os.getenv("HERMES_FEISHU_ACK_EMOJI", "").strip()) else "default"
            logger.info("[Feishu] ACK emoji pool source=%s candidates=%s", source, ",".join(candidates))
            self._ack_emoji_pool_logged = True
    ...
    def _should_batch_text_event(self, event: MessageEvent) -> bool:
        if event.message_type != MessageType.TEXT or event.is_command():
            return False
        chat_type = (getattr(event.source, "chat_type", "") or "").lower()
        if chat_type == "dm":
            return self._env_flag("HERMES_FEISHU_BATCH_DMS", _DEFAULT_BATCH_DMS)
        return True

    def _should_add_ack_reaction(self, event: MessageEvent) -> bool:
        chat_type = (getattr(event.source, "chat_type", "") or "").lower()
        if chat_type == "dm":
            return self._env_flag("HERMES_FEISHU_ACK_REACTION_IN_DMS", _DEFAULT_ACK_REACTION_IN_DMS)
        return self._env_flag("HERMES_FEISHU_ACK_REACTION", True)
    ...
    async def _handle_message_with_guards(self, event: MessageEvent) -> None:
        ...
        async with chat_lock:
            ...
            if message_id and self._should_add_ack_reaction(event):
                chat_type = (getattr(event.source, "chat_type", "") or "").lower()
                if chat_type == "dm":
                    asyncio.create_task(self._add_ack_reaction(message_id))
                else:
    ...
    async def _add_ack_reaction(self, message_id: str) -> Optional[str]:
        ...
        if not self._client or not message_id:
            return None
        emoji_type = self._pick_ack_emoji()
        try:
            ...
            if response and getattr(response, "success", lambda: False)():
                ...
                logger.info("[Feishu] ACK reaction added emoji=%s message_id=%s reaction_id=%s", emoji_type, message_id, reaction_id)
                return reaction_id
            logger.warning(
                "[Feishu] Failed to add ack reaction %s to %s: code=%s msg=%s",
                emoji_type,
                message_id,
    ...
    async def _process_inbound_message(
        ...
        *,
        ...
        if inbound_type == MessageType.TEXT:
            record_ingest, stripped_text = split_record_prefix(text)
            if record_ingest:
                text = stripped_text
                logger.info("[Feishu] Record-ingest prefix detected: message_id=%s", message_id)

        if inbound_type == MessageType.TEXT and text.startswith("/"):
            inbound_type = MessageType.COMMAND

        if inbound_type == MessageType.TEXT and text and not text.startswith("/"):
            link_result = await asyncio.to_thread(
                enrich_text_with_link_content,
                text,
                task_id=f"feishu_{message_id[:24]}",
            )
            if link_result.applied:
                text = link_result.text
                logger.info(
                    "[Feishu] Link enrichment applied: message_id=%s url=%s title=%s error=%s",
                    message_id,
                    link_result.url or "",
                    link_result.title or "",
                    link_result.error or "",
                )
        ...
        async def _timed(label: str, coro: Any) -> tuple[str, Any, float]:
            started = time.perf_counter()
            result = await coro
            return label, result, time.perf_counter() - started

        if chat_type == "p2p":
            chat_info_coro = _resolved(
                {
                    "chat_id": chat_id,
                    "name": chat_id,
                    "type": "dm",
            ...
            chat_info_coro = self.get_chat_info(chat_id)

        reply_coro = self._fetch_message_text(reply_to_message_id) if reply_to_message_id else _resolved(None)
        sender_coro = self._resolve_sender_profile(sender_id, allow_network=(chat_type != "p2p"))

        timed_results = await asyncio.gather(
            _timed("reply_to_text", reply_coro),
            _timed("chat_info", chat_info_coro),
            _timed("sender_profile", sender_coro),
        )
        _timings = {label: elapsed for label, _result, elapsed in timed_results}
        _values = {label: _result for label, _result, elapsed in timed_results}
        reply_to_text = _values["reply_to_text"]
        chat_info = _values["chat_info"]
        sender_profile = _values["sender_profile"]
        logger.info(
            "[Perf] feishu inbound msg=%s step=metadata reply=%.3fs chat=%.3fs sender=%.3fs max=%.3fs",
            message_id,
            _timings["reply_to_text"],
            _timings["chat_info"],
            _timings["sender_profile"],
            max(_timings.values()) if _timings else 0.0,
        )
        ...
        if record_ingest and text:
            await self._handle_record_ingest_message(event=normalized, sender_profile=sender_profile)
            return
        await self._dispatch_inbound_event(normalized)

    async def _fanout_record_ingest(self, *, event: MessageEvent, sender_profile: Dict[str, str]) -> None:
        payload = {
            "text": event.text,
            "feishu_chat_id": getattr(event.source, "chat_id", "") or "",
            "feishu_chat_type": getattr(event.source, "chat_type", "") or "",
            "feishu_user_id": sender_profile.get("user_id", ""),
            "feishu_user_name": sender_profile.get("user_name", ""),
        }
        delivery_id = event.message_id or uuid.uuid4().hex
        try:
            archive_event = build_openclaw_archive_event("feishu_record", delivery_id, payload)
            await asyncio.to_thread(write_openclaw_fanout_file, archive_event, logger)
            try:
                await asyncio.to_thread(persist_phase1_shadow_records, archive_event, logger)
            except Exception:
                logger.warning("[Feishu] lifeos phase1 shadow write failed in fanout for message_id=%s", event.message_id, exc_info=True)
            logger.info("[Feishu] queued record-ingest fan-out event=%s message_id=%s", archive_event.get("event_id", ""), event.message_id)
        except Exception:
            logger.warning("[Feishu] record-ingest fan-out failed for message_id=%s", event.message_id, exc_info=True)

    async def _handle_record_ingest_message(self, *, event: MessageEvent, sender_profile: Dict[str, str]) -> None:
        payload = {
            "text": event.text,
            "record_ingest": True,
            "feishu_chat_id": getattr(event.source, "chat_id", "") or "",
        ...
        try:
            ...
            apply_dailynote_result(archive_event, dailynote_result)
            await asyncio.to_thread(write_openclaw_fanout_file, archive_event, logger)
            try:
                await asyncio.to_thread(persist_phase1_shadow_records, archive_event, logger)
            except Exception:
            ...
            logger.warning("[Feishu] record-ingest direct reply failed for message_id=%s", event.message_id, exc_info=True)
            if archive_event and archive_event.get("dailynote_sync", {}).get("success"):
                reply_text = (
                    "这条已经写入今天的电子报，但后续归档时出了点小问题。\n"
                    "电子报已经落下去了，我接着可以帮你把归档链路修好。"
                )
            else:
                reply_text = (
    ...
    def _cached_sender_name(self, sender_id: Optional[str]) -> Optional[str]:
        if not sender_id:
            return None
    ...
    async def _resolve_sender_profile(self, sender_id: Any, *, allow_network: bool = True) -> Dict[str, Optional[str]]:
        open_id = getattr(sender_id, "open_id", None) or None
        user_id = getattr(sender_id, "user_id", None) or None
        union_id = getattr(sender_id, "union_id", None) or None
        primary_id = open_id or user_id
        lookup_id = primary_id or union_id
        display_name = self._cached_sender_name(lookup_id)
        if display_name is None and allow_network:
            display_name = await self._resolve_sender_name_from_api(lookup_id)
        elif display_name is None and lookup_id and self._loop is not None:
            asyncio.create_task(self._resolve_sender_name_from_api(lookup_id))
        return {
            "user_id": primary_id,
